...
public class BookProvider extends ContentProvider {
    ...
    private static final String AuthorGroup="SELECT "+BOOK_TABLE+"."+BookContract.ID+", "
            ...
            " GROUP BY "+BOOK_TABLE+"."+BookContract.ID+", "+BookContract.TITLE+", "+BookContract.PRICE+", "+BookContract.ISBN;
...
// Create a new one.
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (uriMatcher.match(uri)) {
            case ALL_ROWS:
                // TODOImplement this to handle query of all books.
                Cursor cursor=db.rawQuery(AuthorGroup,selectionArgs);
              //  Cursor testCursor2=db.rawQuery("select * from "+BOOK_TABLE,selectionArgs);
             //   cursor.getCount();
               // testCursor2.getCount();
                // TODO:Remember this one to set notification is very important!!!!!!!!!!!!!!!!!
            ...
            case SINGLE_ROW:
               ...
               // String test="select * from "+BOOK_TABLE;
               // Cursor testCursor=db.rawQuery(test,null);
                //testCursor.getCount();
                //TODO:SelectionArgs are used to replace the syntax ?
                Cursor cursor2=db.rawQuery(AuthorGroup,null);
                cursor2.moveToPosition(id-1);
                //Cursor cursor2=db.query(BOOK_TABLE,null,selection,selectionArg,null,null,sortOrder);
               // cursor2.moveToPosition(id-1);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        // TODOImplement this to handle req uests to delete one or more rows.
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        switch (uriMatcher.match(uri)) {
            case ALL_ROWS:
                int all= db.delete(BOOK_TABLE,selection,selectionArgs);
                getContext().getContentResolver().notifyChange(uri,null);
                return all;
            case SINGLE_ROW:
                //db.delete(BOOK_TABLE,AuthorContract.BOOK_FK+"="+Long.toString(BookContract.getId(uri)),null);
                int single= db.delete(BOOK_TABLE,BookContract._ID + "="+Long.toString(BookContract.getId(uri)),null);
                getContext().getContentResolver().notifyChange(uri,null);
                return single;
            default:
                throw new IllegalArgumentException("Unsupported URI:" + uri);
