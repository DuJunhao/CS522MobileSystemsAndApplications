...
public class MessageContract extends BaseContract {
    ...
    private static int SEQUENCE_NUMBERColumn = -1;
    public static String getSEQUENCE_NUMBER(Cursor cursor) {
        if (SEQUENCE_NUMBERColumn < 0) {
            SEQUENCE_NUMBERColumn = cursor.getColumnIndexOrThrow(ID);
        }
        return cursor.getString(SEQUENCE_NUMBERColumn);
    }

    public static void putSEQUENCE_NUMBER(ContentValues out, String sequence_number) {
        out.put(SEQUENCE_NUMBER, sequence_number);
    }
}
