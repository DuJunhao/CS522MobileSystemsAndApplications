    ...
    public static class DbHelper extends SQLiteOpenHelper {
        ...
        private static final String MESSAGES_CREATE =
                "create table if not exists " + MESSAGE_TABLE+"( "
                        ...
                        +MessageContract.SEQUENCE_NUMBER+" REAL not null,"
                        +PeerForeignKey+" INTEGER NOT NULL,"
                        +"FOREIGN KEY("+PeerForeignKey+") REFERENCES "+PEER_TABLE+"("+PeerContract.Id+") ON DELETE CASCADE); ";

        /*private static final String DB_CREATE="CREATE INDEX MessagesPeerIndex ON "+MESSAGE_TABLE+"("+PeerForeignKey+"); "
...
// Upgrade: drop the old table and create a new one.
    ...
    public Cursor query(Uri uri, String[] projection, String selection,
        ...
        switch (uriMatcher.match(uri)) {
            case MESSAGES_ALL_ROWS:
                ...
                Cursor MESSAGESAllRowsCursor= db.query(MESSAGE_TABLE,
                        new String[]{MessageContract.ID,MessageContract.MESSAGE_TEXT,MessageContract.TIMESTAMP,MessageContract.SENDER,MessageContract.SENDERID,MessageContract.SEQUENCE_NUMBER},
                        selection, selectionArgs, null, null, sortOrder);
                MESSAGESAllRowsCursor.setNotificationUri(getContext().getContentResolver(),uri);
                return MESSAGESAllRowsCursor;
            case PEERS_ALL_ROWS:
                // TODOImplement this to handle query of all peers.
                Cursor PEERSAllRowsCursor= db.query(PEER_TABLE,
                        new String[]{PeerContract.Id,PeerContract.NAME,PeerContract.TIMESTAMP,PeerContract.ADDRESS,PeerContract.PORT},
                        selection, selectionArgs, null, null, sortOrder);
                PEERSAllRowsCursor.setNotificationUri(getContext().getContentResolver(),uri);
                return PEERSAllRowsCursor;
            case MESSAGES_SINGLE_ROW:
                ...
                Cursor MESSAGESSINGLERowCursor=db.query(MESSAGE_TABLE,
                        new String[]{MessageContract.ID,MessageContract.MESSAGE_TEXT,MessageContract.TIMESTAMP,MessageContract.SENDER,MessageContract.SENDERID,MessageContract.SEQUENCE_NUMBER},
                        selection,null,null,null,null);
                MESSAGESSINGLERowCursor.setNotificationUri(getContext().getContentResolver(),uri);
                return MESSAGESSINGLERowCursor;
            case PEERS_SINGLE_ROW:
                // TODOImplement this to handle query of a specific peer.
                //selection=PeerContract.Id+"="+ Long.toString(PeerContract.getId(uri));
               /* Cursor test=db.rawQuery("select * from "+PEER_TABLE,null);
                int i=test.getCount();*/
                Cursor PEERSSINGLERowCursor=db.query(PEER_TABLE,
                        new String[]{PeerContract.Id,PeerContract.NAME,PeerContract.TIMESTAMP,PeerContract.ADDRESS,PeerContract.PORT},
                        selection,null,null,null,null);
                int id=(int)PeerContract.getId(uri);
                PEERSSINGLERowCursor.moveToPosition(id);
                PEERSSINGLERowCursor.setNotificationUri(getContext().getContentResolver(),uri);
                return PEERSSINGLERowCursor;
            default:
                throw new IllegalStateException("insert: bad case");
        }
    }
    ...
    public int update(Uri uri, ContentValues values, String selection,
        ...
        switch (uriMatcher.match(uri)) {
            case MESSAGES_ALL_ROWS:
                int MESSAGESALLROWS=db.update(MESSAGE_TABLE,values,selection,selectionArgs);
                getContext().getContentResolver().notifyChange(uri,null);
                return MESSAGESALLROWS;
            case PEERS_ALL_ROWS:
                //db.delete(BOOK_TABLE,AuthorContract.BOOK_FK+"="+Long.toString(BookContract.getId(uri)),null);
                int PEERSALLROWS=db.update(PEER_TABLE,values,selection,selectionArgs);
                getContext().getContentResolver().notifyChange(uri,null);
                return PEERSALLROWS;
            case MESSAGES_SINGLE_ROW:
                int MESSAGESSINGLEROW=db.update(MESSAGE_TABLE,values,MessageContract.ID+"="+Long.toString(MessageContract.getId(uri)),null);
                getContext().getContentResolver().notifyChange(uri,null);
                return MESSAGESSINGLEROW;
            case PEERS_SINGLE_ROW:
                int PEERSSINGLEROW=db.update(PEER_TABLE,values,PeerContract.Id+"="+Long.toString(PeerContract.getId(uri)),null);
                getContext().getContentResolver().notifyChange(uri,null);
                return PEERSSINGLEROW;
            default:
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (uriMatcher.match(uri)) {
            case MESSAGES_ALL_ROWS:
                int MESSAGESALLROWS = db.delete(MESSAGE_TABLE, selection, selectionArgs);
                getContext().getContentResolver().notifyChange(uri, null);
                return MESSAGESALLROWS;
            case PEERS_ALL_ROWS:
                //db.delete(BOOK_TABLE,AuthorContract.BOOK_FK+"="+Long.toString(BookContract.getId(uri)),null);
                int PEERSALLROWS = db.delete(PEER_TABLE, selection, selectionArgs);
                getContext().getContentResolver().notifyChange(uri, null);
                return PEERSALLROWS;
            case MESSAGES_SINGLE_ROW:
                int MESSAGESSINGLEROW = db.delete(MESSAGE_TABLE, MessageContract.ID + "=" + Long.toString(MessageContract.getId(uri)), null);
                getContext().getContentResolver().notifyChange(uri, null);
                return MESSAGESSINGLEROW;
            case PEERS_SINGLE_ROW:
                int PEERSSINGLEROW = db.delete(PEER_TABLE, PeerContract.Id + "=" + Long.toString(PeerContract.getId(uri)), null);
                getContext().getContentResolver().notifyChange(uri, null);
                return PEERSSINGLEROW;
            default:
                throw new IllegalArgumentException("Unsupported URI:" + uri);
