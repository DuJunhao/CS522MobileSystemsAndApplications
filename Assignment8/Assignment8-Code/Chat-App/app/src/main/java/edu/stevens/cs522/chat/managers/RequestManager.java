...
public class RequestManager extends Manager<ChatMessage> {
    ...
    public void persistDownloadedMsg(ChatMessage message) {
        ContentValues values = new ContentValues();
        Cursor cursor = getSyncResolver().query(PeerContract.CONTENT_URI(message.senderId), null, null, null, null);
        if (cursor.moveToFirst()) {
            Peer peer = new Peer(cursor);
            message.sender = peer.name;
            message.writeToProvider(values);
            getSyncResolver().insert(MessageContract.CONTENT_URI, values);
        }
    }
}
